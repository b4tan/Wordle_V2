...
char **alloc_attempts(int nattempts, int wordbufsz)
{
    ...
    char **arr = malloc(nattempts * sizeof(char*));
    for (int i = 0; i < nattempts; i++) {
        arr[i] = malloc((wordbufsz) * sizeof(char));
    }
...
{
    /* TODO: Implement this function */
    char *p = malloc(wordbufsz * sizeof(char));
...
int main(int argc, char **argv)
{
    ...
    for (int i = 0; i < argc; i++) {
        if (strcmp(argv[i],"./dict/gre-words") == 0) {
            keygen("./dict/gre-words", wordlen, answer);
        } else {
